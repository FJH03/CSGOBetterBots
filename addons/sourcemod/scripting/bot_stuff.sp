...
{
	name = "BOT stuff", 
	author = "manico, modified by [CNSR] FJH_03", 
...
{
    g_smPlayerToTeam = new StringMap();
    ...
    if (!FileExists(szPath))
    {
        ...
        SetFailState("Can't found config");
    }
    ...
    {
        delete kv;
        SetFailState("Unable to parse KeyValues file %s.", szPath);
    }
    ...
    if (kv.GotoFirstSubKey())
    {
        do
        {
            ...
            char players[256];
            kv.GetString("players", players, sizeof(players));
            
            // 分割玩家名字
            char playerNames[32][32];
            int playerCount = ExplodeString(players, ",", playerNames, 32, 32);
            
            for (int i = 0; i < playerCount; i++)
            {
                TrimString(playerNames[i]); // 去除空格
                g_smPlayerToTeam.SetString(playerNames[i], teamName);
            }
        }
...
public void OnClientPostAdminCheck(int client)
{
	...
	if ( g_smPlayerToTeam ) {
		if (g_smPlayerToTeam.GetString(szBotName, szClanTag, sizeof(szClanTag))) {
